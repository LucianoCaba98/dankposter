...
- **`MemeSource` interface** — pluggable sources loaded via `@ConditionalOnProperty`
- **`MemeRenderer` strategy** — source-specific Discord embed rendering
...
Set `SQS_ENABLED=true` plus `SQS_QUEUE_URL`, `SQS_DLQ_URL`, `AWS_REGION`.

### Kafka (event streaming)

Set `KAFKA_ENABLED=true` plus `KAFKA_BOOTSTRAP_SERVERS`, `KAFKA_TOPIC`, `KAFKA_CONSUMER_GROUP`.

See `SQS-KAFKA-PLAYBOOK.md` for full setup instructions.

## Troubleshooting

- **No memes appearing?** Check that at least one source is enabled. Check backend logs for fetch errors.
- **Discord posting fails?** Verify `DISCORD_BOT_TOKEN` and `DISCORD_CHANNEL_ID`. Make sure the bot has "Send Messages" and "Embed Links" permissions.
- **Frontend can't reach API?** Ensure the backend is running on port 8080. The Vite proxy forwards `/api` there.
- **Reddit rate limiting?** The app handles per-subreddit failures gracefully. Reduce subreddits or increase fetch interval if needed.
